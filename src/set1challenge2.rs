pub mod set1challenge2 {
    use std::error::Error;

    pub fn solve(input: &str) -> Result<String, Box<dyn Error>> {
        // decode hex input
        let bytes = hex::decode(input)?;
        // create hex for target to XOR against
        let target = hex::decode(String::from("686974207468652062756c6c277320657965"))?;
        // XOR each byte of the input against the same byte offset in the target
        let result: Vec<u8> = bytes.iter().zip(target.iter()).map(|(&x1, &x2)| x1 ^ x2).collect();
